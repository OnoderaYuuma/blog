...
export const uploadEyecatchImage = async (uploadFile: File) =>{
    const {data, error} = await supabase.storage.from("thumbnails").upload(`imgs/${uploadFile.name}`,uploadFile,{
        cacheControl: "3600",
        upsert: false,
    })
...
export const insertPost = async (title:string, body:string, categoryId:number, eyecatchUrl:string) =>{
    const {error} = await supabase.from("posts").insert({title: title, body: body, category_id: categoryId, eyecatch_url:eyecatchUrl})
    return error;
}
